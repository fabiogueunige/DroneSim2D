...
void sig_handler(int signo)
{
    if (signo == SIGUSR1){
        ...
        writeToLog(debug, "WATCH DOG: received signal");
        server_check = TRUE;
        fclose(debug);
    }
}
...
void sig_handler(int signo, siginfo_t *info, void *context) {
    ...
    if (signo == SIGUSR2) {
        // input and drone has already terminated, so 
        printf("WATCH DOG: quitting program, terminating drone and server...\n");
        kill(server_pid, SIGUSR2);
        kill(drone_pid, SIGUSR2);
        exit(EXIT_FAILURE);
    }
}
...
/*
void proc_anomaly(pid_t pid){
    printf("WATCH DOG: anomaly with process with ID %d", pid);
...
/*
    if (signal(SIGUSR1, sig_handler) == SIG_ERR){
            ...
            writeToLog(errors, "WATCH DOG: error, can't catch SIGUSR1");
    }
...
/*
    int serverpid = atoi(string1);
    int dronepid = atoi(string2);
    int inputpid = atoi(string3);
*/
